...
function FillClock(clock_name, event) {
	var target = event.srcElement ? event.srcElement : event.target;

	if (clock_name + "hour" == target.id) {
		FillClock.hour_is_set = 1;
	} else {
		FillClock.minute_is_set = 1;
	}

	var element_prefix = clock_name.substring(0, clock_name.length - 'clock'.length);

	var hour = getObj(clock_name + "hour") [getObj(clock_name + "hour").selectedIndex].text;
	hour = hour.substring(0, hour.length - 1);
	if ("--" == hour) {
		FillClock.hour_is_set = 0;
	} else {
		getObj(element_prefix + "hour").value   = hour;
	}

	var minute = getObj(clock_name + "minute") [getObj(clock_name + "minute").selectedIndex].text;
	if ("--" == minute) {
		FillClock.minute_is_set = 0;
	} else {
		getObj(element_prefix + "minute").value = minute;
	}
	
	if (1 == FillClock.hour_is_set * FillClock.minute_is_set) {
		HideClock(clock_name);
		with (FillClock) {
			hour_is_set = 0;
			minute_is_set = 0;
		}
...
FillClock.hour_is_set = FillClock.minute_is_set = 0;

function ShowClock(clock_name) {
	var calendar_name = clock_name.substring(0, clock_name.length - 'clock'.length) + "calendar";
	show_obj(clock_name);
}

function HideClock(clock_name) {
	hide_obj(clock_name);
}

function ToggleClock(clock_name) {
	if (state_obj(clock_name) == false) {
		ShowClock(clock_name);
	} else {
		HideClock(clock_name);
	}
}

// fix selectedIndex weirdness in IE5/Mac;
// you may discard it if IE5/Mac is not used 
// by  your intended audience - mg
//window.onload = function() { 
//	hide_obj('nom-du-calendrier-calendar');
//	hide_obj('nom-du-calendrier-clock');
//	};
